...
import org.pemis.facade.ImageFacade;
import org.pemis.facade.PersonFacade;
import org.pemis.facade.WebUserFacade;
import org.pemis.facade.WebUserRoleFacade;
...
//
    ...
    public List<WebUser> getItems() {
        items = getuFacade().findAll("name", true);
        return items;
    }
...
//            JsfUtil.addSuccessMessage("Checking Old Users");
    ...
    private void prepareFirstVisit() {
        ...
        addingRole = new WebUserRole();
        addingRole.setName("Pharmacologist");
        rFacade.create(addingRole);

        addingRole = new WebUserRole();
        addingRole.setName("Tech");
        rFacade.create(addingRole);
        
        addingRole = new WebUserRole();
        addingRole.setName("Bmi");
        rFacade.create(addingRole);        
    ...
    public String manageUser(){
        if(getCurrent()==null || getCurrent().getId()==null||getCurrent().getId()==0){
            JsfUtil.addErrorMessage("Please select a user to manage");
            return "";
        }
        getTransferBean().setWebUser(current);
        return "save_old_user";
    }
    ...
    public String saveUser() {
        ...
        pFacade.edit(getCurrent().getWebUserPerson());
        WebUser user = getCurrent();
        user.setRole(newRole);
        uFacade.edit(user);
        JsfUtil.addSuccessMessage("User Saved.");
        return "";
    }
